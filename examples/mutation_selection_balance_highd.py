...
r  = 1.0/N                          #recombination rate for each interval between loci. 
...
nsamples = 100000                    #increase for better statistics      
...
for locus in range(L):
    ...
    plt.plot(bc, diffusion_theory, color=plt.cm.jet(locus*4), ls='--')
    plt.plot(bc, y, color=plt.cm.jet(locus*4))
