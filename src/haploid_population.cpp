...
#include "popgen.h"
int comp_fit(const void *p1, const void *p2)
{
	fitgt_pair *gt1=(fitgt_pair*)p1;
	fitgt_pair *gt2=(fitgt_pair*)p2;
	if (gt1->fit>gt2->fit) return 1;
	else if  (gt1->fit<gt2->fit) return -1;
	else return 0;
}
...
int haploid_population::allocate_mem()
{
	...
	if (HP_VERBOSE) cerr <<"rec_rates...";
	rec_rates=new sample;
	new_rec_rates=new sample;
	rec_rates->set_up(number_of_individuals);
	new_rec_rates->set_up(number_of_individuals);
	rec_rates->set_range(0,1);
	new_rec_rates->set_range(0,1);
	...
	fitness.set_up(number_of_loci, number_of_words, gsl_rng_uniform_int(evo_generator, 10000000));
	//rec_rates.set_up(number_of_loci, number_of_words, gsl_rng_uniform_int(rng_seed, 100000000));
...
void haploid_population::calc_stat()
{
	...
	if (evolve_rec_rates) rec_rates->calc_variance();
	int i,w,k,locus;
...
double haploid_population::get_multi_point_frequency(vector <int> loci)
{
	...
	for (i=0; i<pop_size; i++)
	{
		for (w=0; w<number_of_words; w++)
		{
			if (genotypes[index(i,w)]&gt_mask[w]==gt_mask[w]) frequency+=1.0;
		}
	}
	if (HP_VERBOSE) cerr<<"done.\n";
	delete [] gt_mask;
	...
	for (i=0; i<pop_size; i++)
	{
		...
		if (total_offsprings+os<number_of_individuals)
		{
			...
			for (o=total_offsprings; o<total_offsprings+os; o++)
			{
				...
				{
					if (gsl_rng_uniform(evo_generator)<rec_rates->values[i]) {sex_gametes[n_sex_gametes]=i; n_sex_gametes++;}
...
int haploid_population::new_generation()
{
	...
	int new_pop_size=0, parent1, parent2, n_o_c=1, no,i,w, stddev;
	//asexual offspring
	for (i = 0; i < n_asex_gametes; ++i) {
		...
		new_rec_rates->values[i]=rec_rates->values[parent1];
	}
	...
	if (n_sex_gametes>0)
	{
		...
		for (i = 0; i < n_sex_gametes-1; i+=2) {
			...
			if (n_o_c>0)
			{
				...
				if (evolve_rec_rates)
				{
					new_rec_rates->values[new_pop_size]=rec_rate.get_func_words(&new_genotypes[index(new_pop_size,0)], number_of_words);
					new_rec_rates->values[new_pop_size+1]=rec_rate.get_func_words(&new_genotypes[index(new_pop_size+1,0)], number_of_words);
				}
				else
				{
					new_rec_rates->values[new_pop_size]=fixed_rec_rate;
					new_rec_rates->values[new_pop_size+1]=fixed_rec_rate;
				}
			}else { //without crossovers, values can be copied
				new_fitness_values->values[new_pop_size]=fitness_values->values[parent1];
				new_rec_rates->values[new_pop_size]=rec_rates->values[parent1];
				new_fitness_values->values[new_pop_size+1]=fitness_values->values[parent2];
				new_rec_rates->values[new_pop_size+1]=rec_rates->values[parent2];
			}
			new_pop_size+=2;
		}
	}
	...
	temp_values=rec_rates;
	rec_rates=new_rec_rates;
	new_rec_rates=temp_values;
...
int haploid_population::recombine_crossover(int parent1, int parent2, int ng)
{
	...
	double crossover_rate=0.5*(rec_rates->values[parent1]+rec_rates->values[parent2]);
...
void haploid_population::calc_individual_fitness(int individual)
{
	...
	if (evolve_rec_rates) rec_rates->values[individual]=rec_rate.get_func_words(&genotypes[index(individual,0)], number_of_words);
	else rec_rates->values[individual]=fixed_rec_rate;
}
...
int haploid_population::add_genotype(int *gt)
{
	...
	if (evolve_rec_rates)
	{
		rec_rates->values[pop_size]=rec_rate.get_func_words(&genotypes[index(pop_size,0)], number_of_words);
	}
	else
	{
		rec_rates->values[pop_size]=fixed_rec_rate;
	}

	fitness_values->number_of_values++;
	rec_rates->number_of_values++;
	pop_size++;
	return 0;
}
...
void haploid_population::calc_rec()
{
	...
	//calculate its fitness and recombination rates
	for (int i=0; i<pop_size; i++){
		if (evolve_rec_rates) rec_rates->values[i]=rec_rate.get_func_words(&genotypes[index(i,0)], number_of_words);
		else rec_rates->values[i]=fixed_rec_rate;
		new_rec_rates->values[i]=rec_rates->values[i];
	}
	rec_rates->calc_variance();
	new_rec_rates->calc_variance();
	if (HP_VERBOSE)
		cerr <<"done"<<endl;
}
...
void haploid_population::shuffle_genotypes()
{
	...
	for (i = 0; i < pop_size; ++i) {
		...
		rec_rates->values[i]=rec_rates->values[asex_gametes[i]];
	}
...
int haploid_population::read_ms_sample(istream &gts, int initial_locus){
	if (gts.bad()){
