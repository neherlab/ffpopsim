...
haploid_highd::haploid_highd(int L_in, int rng_seed, int n_o_traits, bool all_polymorphic_in){
    if (L_in>0){
        set_up(L_in, rng_seed, n_o_traits, all_polymorphic_in);
    }
}
 ...
 */
int haploid_highd::track_locus_genealogy(vector <int> loci, int new_track_genealogy) {
...
unsigned int haploid_highd::flip_single_locus(unsigned int clonenum, int locus) {
	...
	if (track_genealogy) {
		for (unsigned int genlocus=0; genlocus<genealogy.loci.size(); genlocus++) {
			...
			genealogy.newGenerations[genlocus][clonenum].parent_node.location,
                   genealogy.newGenerations[genlocus][clonenum].own_key.location
                    );
...
void haploid_highd::add_clone_to_genealogy(int locusIndex, int dest, int parent, int left, int right, int cs, int n, int parent_location, int own_location){
	...
	genealogy.newGenerations[locusIndex][dest].parent_node.location = parent_location;
	...
	genealogy.newGenerations[locusIndex][dest].own_key.location = own_location;
    ...
    if (number_of_traits != genealogy.newGenerations[locusIndex][dest].traits.size())
    {
        genealogy.newGenerations[locusIndex][dest].traits.clear();
        for (int traitNo = 0; traitNo < number_of_traits; traitNo ++)
        {
            genealogy.newGenerations[locusIndex][dest].traits.push_back(population[dest].trait[traitNo]);
    ...
    }else
    {
        for (int traitNo = 0; traitNo < number_of_traits; traitNo ++)
        {
            genealogy.newGenerations[locusIndex][dest].traits[traitNo] = population[dest].trait[traitNo];
