...
HEADER_MULTIPOP := multi_population.h
SOURCE_MULTIPOP := $(HEADER_MULTIPOP:%.h=%.cpp)
OBJECT_MULTIPOP := $(SOURCE_MULTIPOP:%.cpp=%.o)

SOURCES := $(HEADER_GENERIC) $(HEADER_LOWD) $(HEADER_HIGHD) $(HEADER_HIV) $(SOURCE_GENERIC) $(SOURCE_LOWD) $(SOURCE_HIGHD) $(SOURCE_HIV) $(SOURCE_HIVGENE) $(SOURCE_MULTIPOP)
OBJECTS := $(OBJECT_GENERIC) $(OBJECT_LOWD) $(OBJECT_HIGHD) $(OBJECT_HIV) $(OBJECT_HIVGENE) $(OBJECT_MULTIPOP)
...
$(SRCDIR)/$(LIBRARY): $(OBJECTS:%=$(SRCDIR)/%)
	...
	cp $(HEADER_MULTIPOP:%=$(SRCDIR)/%) $(PKGDIR)/include/
...
$(OBJECT_MULTIPOP:%=$(SRCDIR)/%): $(SOURCE_MULTIPOP:%=$(SRCDIR)/%) $(OBJECT_HIGHD:%=$(SRCDIR)/%) $(HEADER_MULTIPOP:%=$(SRCDIR)/%)
	$(CXX) $(CXXFLAGS) -c -o $@ $(@:.o=.cpp)
...
SWIG_MULTIPOP := multi_population.i
...
$(PYBDIR)/$(SWIG_WRAP) $(PYBDIR)/$(PYMODULE): $(PYBDIR)/$(SWIG_MODULE) $(PYBDIR)/$(SWIG_GENERIC) $(PYBDIR)/$(SWIG_LOWD) $(PYBDIR)/$(SWIG_HIGHD) $(PYBDIR)/$(SWIG_HIV) $(PYBDIR)/$(SWIG_MULTIPOP) $(PYBDIR)/$(SWIG_TYPEMAPS)
